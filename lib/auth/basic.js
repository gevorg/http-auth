 ...
 * @param {String} password store type of passwords.
 */
function Basic(authRealm, authUsers, password) {
	// Realm.
...
Basic.prototype.isAuthenticated = function(request) {
	...
	if("authorization" in request.headers) {
		...
		if(user) {			
			// Encrypt if password is not in plain format.
			if(this.password != "plaintext") {
				var decodedUserTokens = utils.decodeBase64(user).split(":");
				user = utils.base64(htpasswd.encrypt(decodedUserTokens[0], decodedUserTokens[1]));
			}
