...
Basic.prototype.ask = function(response) {
	...
	if(typeof(response.setHeader) == 'function') {
		...
		if (this.proxy) {
			response.write("HTTP/1.1 407 Proxy Access Denied\r\n");
			response.write("Proxy-Authenticate: " + header + "\r\n");
			response.write("\r\n");
			response.write(defaults.HTML_407);
			response.end();
		}
		else {
			response.write("HTTP/1.1 401 Unauthorized\r\n");
			response.write("WWW-Authenticate: " + header + "\r\n");
			response.write("\r\n");
			response.write(defaults.HTML_401);
			response.end();
		}
	}
};
