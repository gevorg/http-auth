...
/**
 * Validates password using hash.
 *
 * @param {String} hash to check.
 * @param {String} password to check.
 * @param {Boolean} true if password is valid, else false.
 */
Basic.validate = function(hash, password) {
		...
		if (clientUserString) {
	      if (this.authHelper) {         
            this.authHelper(clientUserName, function(passwordHash) {
               if ( passwordHash && Basic.validate(passwordHash, clientPasswordHash) ) {
      ...
      for (var i = 0; i < users.length; ++i) {
         ...
         // Ensure the username and password both match.
	      if (myUserName === clientUserName) {
            if (Basic.validate(myPasswordHash, clientPasswordHash)) {
